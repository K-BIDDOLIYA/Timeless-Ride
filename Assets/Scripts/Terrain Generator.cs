...
public class TerrainGenerator : MonoBehaviour
{
    ...
    public float startBehindDistance = 20f;  // terrain begins this far behind the car's spawn X, so its left edge is off-screen

    [Header("Flat Start")]
    public float flatStartLength = 15f;      // length of the safe flat stretch right under the car's spawn point

    private float lastGeneratedX = 0f;
    private float lastCheckpointX = 0f;
    private float flatEndX = 0f;
    private readonly List<GameObject> activeChunks = new List<GameObject>();

    void Start()
    {
        // Ground doesn't move — the car does. So the very first chunk's
        // left edge needs to already sit off-screen behind the camera's
        // starting view, not exactly at the car's spawn X.
        lastGeneratedX = car.position.x - startBehindDistance;
        lastCheckpointX = car.position.x;
        flatEndX = car.position.x + flatStartLength;
    ...
    void GenerateChunk(bool startFlat = false)
    {
        ...
        for (int i = 0; i < pointsPerChunk; i++)
        {
            ...
            float y = (startFlat && worldX < flatEndX) ? groundYOffset : GetGroundHeight(worldX);
